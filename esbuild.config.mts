...
async function buildWebApp() {
  ...
  await build({
    ...
    treeShaking: true,
    banner: {
      js: `/* MikroDocs v${packageVersion} | ${new Date().toISOString()} */`,
    },
  });
}
...
async function buildServiceWorker() {
  ...
  await build({
    entryPoints: [{ in: "./src/serviceWorker/index.ts", out: "service-worker" }],
    outdir: distDir,
    bundle: true,
    format: "iife",
    minify: true,
    ...
    treeShaking: true,
    banner: {
      js: `/* MikroDocs service worker v${packageVersion} | ${new Date().toISOString()} */`,
    },
  });
}
